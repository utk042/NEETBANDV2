...
export default function MobileNavbar({ currentPage, setCurrentPage, user = { isLoggedIn: false } }) {
  const items = [
  ...
  return (
    <nav className="fixed bottom-0 left-0 w-full z-[70] flex justify-around items-center px-1 h-20 bg-surface-container/90 dark:bg-[#121927] backdrop-blur-xl border-t border-outline/10 dark:border-[#1f2838] md:hidden transition-all duration-300 fixed-bottom-safe shadow-[var(--shadow-nav-layout)]">
      {items.map(({ id, label, icon: Icon }) => {
        ...
        return (
          <button
            key={id}
            onClick={() => {
              if ((id === 'dashboard' || id === 'forum' || id === 'course') && !user.isLoggedIn) {
