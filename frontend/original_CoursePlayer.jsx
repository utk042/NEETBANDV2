...
function MathMarkdownContent({ content }) {
  ...
  const processedContent = React.useMemo(() => {
    ...
    let formatted = content.replace(/^[ \t]*[ΓÇóΓû¬ΓùªΓÜ½][ \t]*/gm, '- ');
...
export default function CoursePlayer({ currentTrack, user, onUpgradeClick }) {
  ...
  // ΓöÇΓöÇ OVERVIEW ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
  if (selectedLessonIdx === null || selectedItemIdx === null) {
    return (
      <div
  ...
  // ΓöÇΓöÇ LESSON VIEW (Playing an item) ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
  const lesson = lessons[selectedLessonIdx];
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-1 overflow-hidden relative">
        ...
        {/* Sidebar ΓÇö accordion content tree */}
        <aside className={`
          ...
          <div className="max-w-3xl mx-auto w-full">
            ...
            {locked ? (
              ...
              <>
                ...
                {/* ΓöÇΓöÇ Q&A FLASHCARD RENDER ΓöÇΓöÇ */}
                {item?.type === 'qa' && (
                  <div>
                        ...
                        >
                          ...
                          <div className="flashcard-scene">
                            <div
                              ...
                              aria-label={flashcardFlipped ? 'Showing answer ΓÇö tap to see question' : 'Showing question ΓÇö tap to reveal answer'}
                            >
                              {/* FRONT ΓÇö Question */}
                              <div
                          ...
                          <p className="hidden sm:block text-center text-[10px] text-on-surface-variant/25 mt-3 font-mono tracking-wide">
                            ΓåÉ ΓåÆ to navigate ┬╖ space to flip
                          </p>
                        </div>
                      );
                    })()}
                ...
                {/* ΓöÇΓöÇ NOTES / TEXT RENDER ΓöÇΓöÇ */}
                ...
                {/* ΓöÇΓöÇ SONG/AUDIO RENDER ΓöÇΓöÇ */}
                {item?.type === 'song' && (
                ...
                {(item?.type === 'notes' || item?.type === 'lesson' || item?.type === 'reading') && !activeDetails?.content && !item?.fileUrl && (
                  <div className="flex flex-col items-center justify-center py-20 text-center">
                    ...
                    <p className="text-xs text-on-surface-variant/60 mt-1">Content coming soon ΓÇö the instructor hasn't added details yet.</p>
                  </div>
                )}
              </>
            )}
